...
% mesh.ygrid(2:mesh.nny-1)
% mesh.xmol(2:mesh.nny-1)
% mesh.dop(2:mesh.nny-1)
% 
% 'Check variables VENUS',keyboard
...
mode.elecABS=mode.elec*1e-18;
mode.holeABS=mode.hole*1e-18;
...
% Pointer to match D1ANA reducing matrices to a single column
purid=[];
for ii=1:3
    purid=[purid [[1:mesh.nny]+(ii-1)*mesh.nny*mesh.nnx]];
end
purid1=purid+mesh.nny;
pulast=[purid(end)+mesh.nny+[1 2]]; % Points to circuit equations
PUN.purid=purid;
PUN.purid1=purid1;
PUN.pulast=pulast;
...
%             [Kmat0,Jmat0,Jmat2,uvet,rvet,mode]=assem(geom,mesh,mode,uvet,v0_dd);
            [Kmat0_old,Jmat0_old,Jmat2_old,uvet_old1,rvet_old,mode,tvet_old]=assem_GBT(geom,mesh,mode,uvet,v0_dd);
        ...
        Jmat0=MatrixCompacter(Jmat0_old,PUN);
        Kmat0=MatrixCompacter(Kmat0_old,PUN);
        Jmat2=MatrixCompacter(Jmat2_old,PUN);
        
        uvet=MatrixCompacter(uvet_old1,PUN);
        rvet=MatrixCompacter(rvet_old,PUN);
        ...
        if(res<mode.tolconv), mode.iconv(indv)=1; mode.res(indv) = res; 
            uvet=VectorUnpacker(uvet,PUN);
            break
        end
