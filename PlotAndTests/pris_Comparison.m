...
if length(iLoad)==0
    ...
    % save nomeLoopFinito nomeSave
    nomeSave=input(' Inserire nome _pmat file da confrontare ');
    eval(['load  ',nomeSave,'_pmat'])
    fibar=strfind(nomeSave,'\');
    nomeSave=nomeSave(fibar(end)+1:end);
end
...
for IPAR=IPvet
    ...
    eval(['exi=exist( ''',strSave,'_',nome,'_',num2str(IPAR),'.mat',''');'])
    
    if exi==0
        return
    end
    eval(['load ',strSave,'_',nome,'_',num2str(IPAR),'.mat'])
...
%                 eval(['load MarkusN_',num2str(Isize),'_T',num2str(T0),'.mat'])
                eval(['load MarkusN_4_T',num2str(T0),'.mat'])
            else
...
%         else
        ...
        plot(I(pWPE:end),P(pWPE:end)./(I(pWPE:end).*V(pWPE:end))*100,[colo(kpar),'.'],'linewidth',2,'markersize',18)
        plot(Imeas(paWPE),Lmeas(paWPE)./(Imeas(paWPE).*Vmeas(paWPE))*100,[colo(kpar),'o'],'linewidth',1,'markersize',5)
        end
        pausak
        
        iPlotDyn = input('Vuoi vedere i risultati analisi piccolo segnale? (ENTER, Sì; 0, NO) ')
        if length(iPlotDyn)==0
        % PlotDyn
        Dyn_Comparison
        end
...
%             plot(Cur,P_dB+scLo,[Smis(k),'--'])
    ...
    lines{1}='-'; lines{2}='--'; lines{3}='-.';  lines{4}='.';  lines{5}='+';  lines{6}='o';
    for k=1:length(MODEplot)
        plot(Imod{k},Pmod{k},lines{mod(k,7)},'linewidth',2)
        hold on
        ax = gca;
        ax.ColorOrderIndex = 1;
    end
...
%% Possibility of superimposing other data
iComp=input(' Vuoi confrontare con altri dati? [Enter: NO;  1: YES] ')
...
%         semilogy(Imeas_res(pu),Rmeas(pu),[colo(kpar),'o'],'Markersize',3)
        ...
        %semilogy(mode.ii_dd(lres)*1000,Resistence(lres),[colo(kpar),'.-'],'Markersize',10)
%         semilogy(mode.ii_dd(lres)*1e3,Resistence(lres),[colo(kpar),'.-'],'linewidth',2)
        semilogy(mode.ii_dd(lres)*1e3,Resistence(lres),'.-','linewidth',1)
...
%         ylim([30,500])
    ...
    legend('1D-dyn','VENUS-dyn','exp-static','VENUS-static','VENUS-noTerm')
end