...
for IPARlo=1:REP
 IPlo=IPARlo+IP0;
 rad_settingV{IPlo}=[radi,num2str(IPARlo)];
 PMAT{IPlo}=PMAT{IPAR0};
 NP(IPlo)=length(PMAT{IPAR0});
end
...
for IPARlo=1:REP
 IPvetRep=IPARlo+IP0;
 PMAT{IPvetRep}=PMAT{IPAR0};
 NP(IPvetRep)=length(PMAT{IPAR0});
 Svar{IPvetRep}=Svar{IPAR0};
end
