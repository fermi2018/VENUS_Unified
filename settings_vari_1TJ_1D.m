...
if IPAR~=11 
 T0=273+Temperature;         % environment temperature, K
  ...
  T0a=T0-273;
    ...
    elseif  T0a==20 
     Imassimo=10;  % massima corrente analizzata
     Vadd=[1.55:0.05:2.8];   
    end  
...
%  mode.heteromode=2; % if 2 puts the "heteropoints"; if 1 doesn't
 ...
 mode.CarrierNorm=1e8; % 1D: 1e8; 3D: 1; 1e6 above
...
%             OX=10.50*ones(1,Isize);    % DIAMETERS (D1ANA - MDPIAS) - TJ fitting of 3D case 
 ...
 if mode.quasi1D==1 && mode.flgBTJ==1
    mode.fPdifScaling=1.35; % 1 for Oxide or 3D simulations; 1.2 for TJ (1D)
 else
    mode.fPdifScaling=1; % 1 for Oxide or 3D simulations; 1.2 for TJ (1D)
 end
 
STR{11}='MarkusN_FINALEdisFitto';   % top DBR medio
STR{12}='MarkusN_TopLAqw2';

STR{13}='MarkusN_BTJ';
STR{14}='MarkusN_TJ_oxAbove';
STR{15}='MarkusN_TJ_oxBoth';
STR{16}='MarkusN_TJ_oxBelow';

STR{17}='Resistor_ox';

STR{18}='MarkusN_TJ_oxBothGraded';
STR{19}='MarkusN_TJ_oxBelowGraded';
STR{20}='MarkusN_TJ_oxAboveGraded';

STR{21}='MarkusN_1TJetch'; % single TJ case, obtained FROM the 2TJ structure
...
mode.maxiter=20; % maximum number of iterations (DD+PB)  
...
mode.minthermalvoltage=1.5; % min. voltage to activate thermic simulator
