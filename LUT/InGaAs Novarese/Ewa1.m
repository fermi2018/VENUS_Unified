...
%==========================================================================
mode.fileNameLUT='Anders1060-6nmQw318In-Bar10nm_19Dic19ren'; % filename for LUT
...
mode.FatRen=.2; % enable gap renormalization computation
...
% Densityv=[ 0.01 5 10]*1e12;
 ...
 Densityv=linspace(.5,5,26); % for PL plot
 %Densityv=linspace(0,5,4); % for PL plot
 Densityv(1)=1e-4;
 Densityv=Densityv*1e12;
 %Densityv=[1e-5 1e-4 1e-3 0.01 0.1  0.3:.3:3 4:12]*1e12;
 
 Densityv=[.01 1 2 3 4 5];

  Densityv=linspace(0.5,5,31); % for PL plot
  
  %Densityv=linspace(3.5,5,31); % for PL plot
  %Densityv(1)=1e-4;
 Densityv=Densityv*1e12;
...
lambdavet=[1055:5:1065]*1e-9;% linspace(830,870,41)*1e-9;
...
eval(['save ',mode.fileNameLUT,' lambda N Tvet G Rsp Es Dep EFspV'])
lambda=Densityv;
if length(size(G))==3
  plot(lambda,G,'.-')
...
figure, plot(Densityv(1:end-1),diff(nn)./diff(gg))
%ylim([-10 0])
ylabel('Henry factor')
xlabel('Carrier Dens')  
if length(Tvet)>1    
