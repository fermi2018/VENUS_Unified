function [lmbV,xvV,lmbC,xvC]=solve_kp44_AlGaAs(mesh,kx,ky,mode)
%
...
for ie=1:mesh.ne % Loop over mesh elements
    ...
    % bulk Hamiltonian
    [H0,H1l,H1r,H2]=assem_kp44_AlGaAs(kx,ky,mesh.xmol(ie),mode);
    %
    H0 = H0 + mesh.evb(ie)*I;
...
% % 
% % for indLivello=1:2
% xv = reshape(xvC(:,1),4,mesh.nn);
% plot(mesh.x,abs(sum(xv,1)))
% % hold on
% % plot(mesh.x,abs(xvCtmps(:,indLivello)),'k')
% % end
