function [lmbV,xvV,lmbC,xvC,mesh]=solve_kp44_InGaAs(mesh,kx,ky,mode)
%
...
for ie=1:mesh.ne % Loop over mesh elements
    ...
    % check if we are in well or barrier
    if (abs(mesh.evb(ie))<1e-10); %
    [H0,H1l,H1r,H2,Egtot,mesh]=assem_kp44_InGaAs(mesh,kx,ky,mesh.xmol_well,mesh.strain_w,mode);
    else
     [H0,H1l,H1r,H2,Egtot,mesh]=assem_kp44_GaAsP(mesh,kx,ky,mesh.xmol_barrier,mesh.strain_b,mode);
    end
...
% compare Eg
%figure,plot(mesh.xc,mesh.Eg+mesh.ecb,'linewidth',2)
%hold on
%plot(mesh.xc,Eg+mesh.evb,'--','linewidth',2)
