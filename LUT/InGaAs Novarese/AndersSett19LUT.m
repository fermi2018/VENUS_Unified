...
%==========================================================================
mode.fileNameLUT='Anders1060-6nmQw27In-Bar10nm_26Set19LUT'; % filename for LUT
...
lambdavet=linspace(1040,1070,11)*1e-9;

%lambdavet=linspace(1000,1080,101)*1e-9;
%Tvet=[290];
%Densityv=[1e-4 ]*1e12;
%lambdavet=1040*1e-9;
...
%==========================================================================
mesh.fileName=['subbands_WQW=',num2str(mesh.Lz*1e9),'-',num2str(BB),'nm_',num2str(mesh.num_kvectors),'k_',num2str(mesh.max_k),'kMax.mat'];
%
...
FOR='parfor indT=1:length(Tvet)';
FOR='for indT=1:length(Tvet)';
...
%     'rixc', keyboard
    for indlambda=1:length(lav)
     ...
     Gtemp=squeeze(DeltaN(:,:,indlambda,indTemp));     
     GtempdE=squeeze(DeltaNE(:,:,indlambda,indTemp));
     GtempdH=squeeze(DeltaNH(:,:,indlambda,indTemp));
     dGE=(GtempdE-Gtemp)*iDele;
     dGH=iDlac*(GtempdH-Gtemp);
     dndE(:,:,indlambda,indTemp)=dGE;  
     dndH(:,:,indlambda,indTemp)=dGH;      
...
var=' G dGdE dGdH lav por_E por_H porE_Rsp porH_Rsp Ric dRicdE dRicdH Es dEdE dEdH Tv DeltaN dndE dndH';
