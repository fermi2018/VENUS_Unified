...
function [H0,H1L,H1R,H2,Egtot,mesh]=assem_kp44_GaAsP(mesh,kx,ky,xmol,strain,mode)
%
...
% Material parameters
%Eg=1.412*qel;
 Eg = mesh.EgGaAs*qel; % Energy gap, fitted from Gerlach PL data, J
...
% due sch per le due tipi dei masse leggere e pesanti, buca aggiornata

gamma_bar = (gamma2+gamma3)/2;
C12=5.376;  %Zwang dyn/cm^2
C11=12.879; %Zwang dyn/cm^2
ac=-7.17*qel; % eV
av=2.16 *qel; %eV
b=-1.7*qel; %eV
exx=strain;
...
% Material parameters
Eg=mesh.EgAlAs*qel;
%  Eg = 2.776*qel; % Energy gap of GaP direct from whole forumla of InGaAsP
%  Eg=  2.706530530530530*qel;
  Eg_AlAs = 2.776; % Energy gap of GaP direct from whole forumla of InGaAsP
...
gamma_bar = (gamma2+gamma3)/2;
C12=5.34;  %Zwang dyn/cm^2
C11=12.5; ;  %Zwang dyn/cm^2
ac=-5.64*qel; % eV
av=2.47 *qel; %eV
b=-1.5*qel; %eV
exx=strain;
...
U = U *1/SR2; 
%
H0_AlAs =  (inv(U.')*(H0_AlAs) *(U.'));

H2_AlAs =  (inv(U.')*(H2_AlAs) *(U.')); 
H1_AlAsL = (inv(U.')*(H1_AlAsL)*(U.')); 
H1_AlAsR = (inv(U.')*(H1_AlAsR)*(U.')); 

if mode.fgain==0
H0_AlAs  = H0_AlAs(ind1,ind1); 
H2_AlAs  = H2_AlAs(ind1,ind1); 
H1_AlAsL = H1_AlAsL(ind1,ind1); 
H1_AlAsR = H1_AlAsR(ind1,ind1); 
else
H0_AlAs  = H0_AlAs(ind2,ind2); 
H2_AlAs  = H2_AlAs(ind2,ind2); 
H1_AlAsL = H1_AlAsL(ind2,ind2); 
H1_AlAsR = H1_AlAsR(ind2,ind2); 
end
% =========================================================================
% GaAsP hamiltonian interpolation
% =========================================================================
H0 =  xmol*H0_GaAs  +(1- xmol)*H0_AlAs;
H1L =xmol*H1_GaAsL + (1-xmol)*H1_AlAsL;
H1R =xmol*H1_GaAsR + (1-xmol)*H1_AlAsR;
H2 =  xmol*H2_GaAs  +(1- xmol)*H2_AlAs;
Egtot =  xmol*Eg_GaAs  +(1- xmol)*Eg_AlAs;
% Eg_GaAsP=2.776-1.469*xmol+0.108*xmol^2;
% Egtot=Eg_GaAsP;
% H0(1,1)=H0(1,1)+Eg_GaAsP;
