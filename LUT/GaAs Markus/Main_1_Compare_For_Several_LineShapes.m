...
mode.iline='Landsberg_modified'; % 'nMark' ; 'lorentzian'; 'Landsberg'; Landsberg_extended' and Landsberg_modified  models 
...
IL{2}='nMark';
IL{3}='Landsberg';
IL{4}='Landsberg_extended';

for indT=1:length(IL) % temperature loop
mode.iline=IL{indT}; % 'nMark' ; 'lorentzian'; 'Landsberg' or Landsberg_extended'  models    
    [GT,EsT,DepT,RspT,eD,hD,EFc,EFv,Ren]=pf_functionFit(eDensityv,hDensityv,lambdavet,Tvet,1,Ban,mesh,mode,Pargain);
...
for k=1:length(eDensityv), 
    ...
    plot(lambdavet*1e9,squeeze(G(k,:,:)),'linewidth',2), 
end
% ylim([-500, 1500])
% grid, pausak
