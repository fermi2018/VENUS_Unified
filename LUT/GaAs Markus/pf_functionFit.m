function [G,Es,Dep,Rsp,eD1,hD1,EFcv,EFvv,Ren]=pf_functionFit(eDensityv,hDensityv,lambda,Tvet,indT,Ban,mesh,mode,Pargain)
...
for idh=1:length(hDensityv)
    ...
    for ide=1:length(eDensityv)
        ...
        [EFc] = fminbnd(@(EFc) f_rho(kgrid,SBC,EFc,kBTev,eDensity,C0+mesh.Eg,mode), 0,mesh.Eg+C0*3,OPTIONS);
        ...
        ECV_tot=DelGap+ECV_tot0; % applying gap renormalization
...
%                'verG', keyboard
        ...
        if(mode.LUT==0)
            EFsave(idh,ide)=EFvv(ide,idh)+EFcv(ide,idh)+hbar.*DelGap(1,1)./qel-DeltaETemp;
