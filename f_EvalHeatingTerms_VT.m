...
if isfield(mesh,'IBTJ')==1
    for iTJ=1:size(mesh.BTJcell,1)
  
      %  sigma(mesh.LBTJ(iTJ):mesh.RBTJ(iTJ))=mode.sigmaTJ(iTJ)/mode.CarrierNorm;
      %  sigma(sort(unique([mesh.IBTJ{1,:}])))=mode.sigmaTJ(iTJ)/mode.CarrierNorm;
      sigma(sort(unique([mesh.IBTJ{iTJ,:}])))=mode.sigmaTJ(iTJ); 
...
if(mode.oflg)
    ...
    mode.HeatOptAbs=mode.HeatOptAbs;
