function [Efn2D]=mpinvferdr_n2D(n2D,Em,Ec0,Nc2D,Vt,LowerBound,UpperBound)
%
% function [Efn2D]=invferdr_n2D(n2D,Em,Ec0,Nc2D,Vt,LowerBound,UpperBound)
%
% This function is aimed at computing the numerical inverse of ferdr2D.
...
LHS=n2D;
Efn2D_GuessForNewton=(LowerBound+UpperBound)/mp('2');
% Setting to inf to enter in the loop at least once
GuessPreviousStep=inf*ones(size(Efn2D_GuessForNewton),'mp');
...
while(max(abs(Efn2D_GuessForNewton-GuessPreviousStep))>tolBisection)
    ...
    xnum=(ones(NBound_C,1,'mp')*UpperBound-Em)./Vt; 
    xden=(ones(NBound_C,1,'mp')*UpperBound-Ec0)./Vt;
    [n2D_UB,dum] = mpferdr2D(xnum,xden);
    n2D_UB=sum(Nc2D.*n2D_UB,1);
    fU=n2D_UB-LHS;
    %
    % Calculation of the lower bound for the current iteration
    xnum=(ones(NBound_C,1,'mp')*LowerBound-Em)./Vt; 
    xden=(ones(NBound_C,1,'mp')*LowerBound-Ec0)./Vt;
    [n2D_LB,dum] = mpferdr2D(xnum,xden);
    n2D_LB=sum(Nc2D.*n2D_LB,1);
    fL=n2D_LB-LHS;
    %
    % Calculation of the guess for the current iteration
    xnum=(ones(NBound_C,1,'mp')*Efn2D_GuessForNewton-Em)./Vt; 
    xden=(ones(NBound_C,1,'mp')*Efn2D_GuessForNewton-Ec0)./Vt;
    [n2D_C,dum] = mpferdr2D(xnum,xden);
...
expnum=exp((ones(NBound_C,1,'mp')*Efn2D_GuessForNewton-Em)./Vt);
expden=exp((ones(NBound_C,1,'mp')*Efn2D_GuessForNewton-Ec0)./Vt);
...
while(max(abs(Upgrade))>tolNewton && NumberOfIterations<MaxIterNewton) % check performed on the solution
    ...
    expnum=exp((ones(NBound_C,1,'mp')*Efn2D_GuessForNewton-Em)./Vt);
    lognum=log(mp('1')+expnum);
    ddlognum_Efn2D=-expnum./(mp('1')+expnum)./Vt;

    % "denominator" exponential and terms
    expden=exp((ones(NBound_C,1,'mp')*Efn2D_GuessForNewton-Ec0)./Vt);
    logden=log(mp('1')+expden);
    ddlogden_Efn2D=-expden./(mp('1')+expden)./Vt;
    
    % Test, debug case: no "denominator" terms
    % f=LHS-sum(Nc2D.*lognum,1);
    % df=sum(Nc2D.*ddlognum_Efn2D,1);
    
    % Functional to be minimized: definition (f = LHS - Nc2D*ferdr2D)
