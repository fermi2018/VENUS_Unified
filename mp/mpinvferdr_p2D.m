function [Efp2D]=mpinvferdr_p2D(p2D,Ed,Ev0,Nv2D,Vt,LowerBound,UpperBound)
%
...
while(max(abs(Efp2D_GuessForNewton-GuessPreviousStep))>tolBisection)
    ...
    [p2D_UB,dum] = mpferdr2D(xnum,xden);
    ...
    [p2D_LB,dum] = mpferdr2D(xnum,xden);
...
while(max(abs(Upgrade))>tolNewton && NumberOfIterations<MaxIterNewton) % check performed on the solution
    ...
    % Functional to be minimized: definition (f = LHS - Nv2D*ferdr2D)
