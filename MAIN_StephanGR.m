...
iStruttura=51; % Gr RELIEF ok per prove metallo
...
radi='_Stephan_FIT_Torrelli'; % OC-VCSEL, 1D: use for comparison with D1ANA (Torrelli Fitted data)
...
%         clear mode VELMInfo VELMInput VO
        ...
        if IPAR>0
            ...
            eval([stringa_parametro,';']) % Torrelli: cambia la temperatura
            end
        end
        eval(['settings_vari',rad_setting])
...
if IPLOT==1
    ...
    hold on
    IndexLegend = 0;
    % for IndVelm = [1, floor(length(VELMInfo)/3), floor(2*length(VELMInfo)/3), floor(length(VELMInfo))]
    % for IndVelm = 1:floor(length(VELMInfo)/2)
    for IndVelm = 10:5:40
        IndexLegend = IndexLegend + 1;
        plot(VELMInfo(IndVelm).FF_ApertureAngle,VELMInfo(IndVelm).FF_Intensity,'LineWidth',1.5)
        LegendCell{IndexLegend} = ['I = ',num2str(1e3*mode.ii_dd(VELMInfo(IndVelm).indVoltage)),' mA'];
    end
    hold off
    grid on
    legend(LegendCell)
