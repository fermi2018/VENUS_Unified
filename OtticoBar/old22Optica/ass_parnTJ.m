...
% per struttura non planare e/o  anisotropa
 ...
 global Fat_perd iPerdRegione iPerdSoglia Xperd  Fat_perd_Sog Fat_perdTJ
...
%  'iper', keyboard
  ...
  if Fat_perd~=1
   ...
   if Fat_perd>=0 & iPerdRegione+iPerdSoglia==0
   nip=nitn;
   fin=find(abs(imag(nip))<.3);
   nirid=nip(fin);
   nirid=real(nirid)+1i*imag(nirid)*Fat_perd; 
   nitn(fin)=nirid;   
   else
   ...
   if iPerdRegione==0
    fipe=1:length(xp);
   elseif iPerdRegione==1
    fipe=find(xp>=Xperd(3) & xp<=Xperd(4));
   elseif iPerdRegione==2
    fipe=find( (xp>=Xperd(3) & xp<=Xperd(5)) | (xp<=Xperd(2) & xp>=Xperd(1)) );  
   elseif iPerdRegione>=3
    fipe=find( (xp>=Xperd(4) & xp<=Xperd(5)) | (xp<=Xperd(2) & xp>=Xperd(1)) );  	
	fipeTJ=find( (xp>=Xperd(2) & xp<=Xperd(4)));  	
	
   end

%   figure, plot(xp,nitn(1,:),xp,-imag(nitn(1,:))*30+2,'r.'), hold on
%   plot(xp(fipe),-imag(nitn(1,fipe))*30+2,'go'), 
%   keyboard
   
     if iPerdSoglia==1 
      nipD=imag(nitn(1,fipe));
	  finoz=find(nipD~=0);
	  PerSog=max(nipD(finoz))*Fat_perd_Sog;
	 else  
	  PerSog=0;
	 end 
      nip=nitn(:,fipe);	  
	  ImaRef=imag(nip)*abs(Fat_perd); 
	  fiSog=find(ImaRef>PerSog & ImaRef~=0);
	  ImaRef(fiSog)=PerSog;
	  
%   figure, plot(xp,nitn(1,:),xp,-imag(nitn(1,:))*30+2,'r.'), hold on
   
%   figure, plot(xp(fipe),log10(-imag(nitn(1,fipe))),'go'), hold on
%   plot(xp(fipe),log10(-ImaRef(1,:)),'c+'), 
%   keyboard

      nitn(:,fipe)=real(nitn(:,fipe))+1i*ImaRef;
	  if iPerdRegione==4
       nitn(:,fipeTJ)=real(nitn(:,fipeTJ))+1i*imag(nitn(:,fipeTJ))*Fat_perdTJ;
	  end 
...
%' qui perd', keyboard
  ...
  if Fat_perd~=1
     ...
     if iPerdSoglia==1 
      ...
      nipD=nib(1,fipe);
	  PerSog=max(imag(nipD))*Fat_perd_Sog;
	 else  
	  PerSog=0;
	 end
...
%   global nv_old nv
   ...
   nv=[nv_old(1,:); nitot; nv_old(end,:)];  
   csL=cumsum(Litot);
     cpe=4e4*pi/lambda;
  if ifp==-10	 
%   figure, plot(csL,log10(-cpe*imag(nv_old(2:end-1,1))),'o',csL,log10(-cpe*imag(nv(2:end-1,1))),'+')
%   'asspar verifica perdite',   keyboard
  end 
