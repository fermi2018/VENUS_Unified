...
pice00=find(abs(cce)==0);

ccep=ccesav(pice).';
ccep=[ccep; -ccep];
ccep=[ccep; conj(ccep)];
Rvpx=repmat(Rvetx(pice),1,4);
Rvpy=repmat(Rvety(pice),1,4);

if length(pice0)>0
 ficed=find((real(cce)==0) & (abs(cce)~=0));
 if length(ficed)>0
  ccep=[ccep; cce(ficed); -cce(ficed)];
  Rvpx=[Rvpx repmat(Rvetx(ficed),1,2)];
  Rvpy=[Rvpy repmat(Rvety(ficed),1,2)];
 end
 ...
 if length(pice00)>0
  ccep=[ccep; cce(pice00)];
  Rvpx=[Rvpx Rvetx(pice00)];
  Rvpy=[Rvpy Rvety(pice00)];
 end
...
%  ro=ccep(nh)+R*(1+Delta*cos(4*fi)).*exp(j*fi);
  ro=sha_gen(sh_type,fi,Rvp(nh),Delta,ccep(nh));
  Rvd=[Rvd ro];
 end
...
% ro=cce0(nh)+R*(1+Delta*cos(4*fi)).*exp(j*fi);
 ro=sha_gen(sh_type,fi,Rv0(nh),Delta,cce0(nh));
 M=abs(ro);
 F=angle(ro);
 fiF=find(F<0);
 F(fiF)=F(fiF)+2*pi;
...
% rod0=abs(cce(fice0)+R*(1+Delta*cos(4*fi00)).*exp(j*fi00));
 rod0=abs(sha_gen(sh_type,fi00,Rvet(fice0),Delta,cce(fice0)));
else
 rod0=[];
end
...
%  ropd=cce(ic)+R*(1+Delta*cos(4*fi)).*exp(j*fi);
  ropd=sha_gen(sh_type,fi,Rvet(ic),Delta,cce(ic));
 else
%  ropd=cce(ic)+R*(1+Delta*cos(4*fi00)).*exp(j*fi00);
  ropd=sha_gen(sh_type,fi00,Rvet(ic),Delta,cce(ic));
 end
