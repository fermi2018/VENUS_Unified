%' calc_new per pu2 QUESTO ', keyboard
...
%' verifica Amedio prima', keyboard
...
%      Afz_s=Afzc;
  ...
  %    Oo=inv(OoScala);
   ...
   Ynore0=1./[Zve];
   if iLP==1
    Ynore0=ones(size(Zve));
    Ynorm0=ones(size(Zve));
   else 
    Ynorm0=1./[Zvm];
   end
...
%   'Ybor', keyboard
   ...
   Anout=(Afz(:,iFFsect)+Afzf(:,iFFsect));
   Anouth= YD*(Afz(:,iFFsect)-Afzf(:,iFFsect));;   
   if length(ficrit)>0 & iFFsect==1
    Anout=Trcrit*Afz(:,1).*(Gas+1);
    Anouth= YD*Trcrit*Afz(:,1).*(1-Gas);
   end
   Anoute=Anout;
