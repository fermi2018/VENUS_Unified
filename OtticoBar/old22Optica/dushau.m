...
%  ro=ccep(nh)+R*(1+Delta*cos(4*fi)).*exp(j*fi);
  ro=sha_gen(sh_type,fi,R,Delta,ccep(nh),ce0,fi1q,fi4q);
  Rvd=[Rvd ro];
 end
...
% ro=cce0(nh)+R*(1+Delta*cos(4*fi)).*exp(j*fi);
 ro=sha_gen(sh_type,fi,R,Delta,cce0(nh),ce0,fi1q,fi4q);
 M=abs(ro);
 F=angle(ro);
 fiF=find(F<0);
 F(fiF)=F(fiF)+2*pi;
...
% rod0=abs(cce(fice0)+R*(1+Delta*cos(4*fi00)).*exp(j*fi00));
 rod0=sha_gen(sh_type,fi00,R,Delta,cce(fice0),ce0,fi1q0,fi4q0);
else
 rod0=[];
end
...
%  ropd=cce(ic)+R*(1+Delta*cos(4*fi)).*exp(j*fi);
  ropd=sha_gen(sh_type,fi,R,Delta,cce(ic),ce0,fi1q,fi4q);
 else
%  ropd=cce(ic)+R*(1+Delta*cos(4*fi00)).*exp(j*fi00);
  ropd=sha_gen(sh_type,fi00,R,Delta,cce(ic),ce0,fi1q0,fi4q0);
 end
...
%   ropdui=cce(ic)+R*(1+Delta*cos(4*fi0)).*exp(j*fi0);
   ropdui=sha_gen(sh_type,fi0,R,Delta,cce(ic),ce0,fi1q,fi4q);
