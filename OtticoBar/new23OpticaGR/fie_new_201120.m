pograp=[220   -40   651   843];
pogram=[220   -40   651   843];
if exist('SPP')==0
 SPP=0;
end
% 'entro fie_new 2011', keyboard
if ifp==-10
...
if iskim==0
 ...
 if length(if_only_out)==0
   ...
   Gsov(imod)=glostotal/NQW_ef;
   NQW_efVet(imod)=NQW_ef;
   Fsov(imod)=ze;
...
iskv=0;
   ...
   if iskv==0
    ...
    aax=1.5*aiat(1);
%    aax=2*min(aytot(find(aytot(:,1)>0,1)));
...
%      stri=[stri,' gain per QW = ',num2str(glosp/(NQW*fatqw),'%0.4e')];
      ...
      stri=['lam= ',num2str(laver,'%0.4e'),',     KMAX = ',num2str(alim,'%0.2f'),',  Nk = ',num2str(length(KK),'%3i'),', N-acc = ',num2str(numodiacc,'%2i')];
%      stri=[stri,' gain per QW = ',num2str(glosp/(NQW*fatqw),'%0.4e')];
      text(-.25,1.08,stri);
...
%        map_fnew(XP,YP,real(Exdu),aax,Cug.x,Cug.y,Cug.z,titl,ibar,iaoff)
       ...
       if iEz==1
        mXe=abs(max(max(E2xp)));
        mYe=abs(max(max(E2yp)));
%        'qui', keyboard
        if mXe>mYe
         E2mp=E2xp;
         titlm=' E_x  QW';
