...
fPmax=10;
fPmax=0;
iexpe=0;      %=1 usa interp su alvet e gvet
inor=0;
if ~exist('ipost')
 ipost=0;
end 
...
%GMA=1e6;
%GMA=1e12;
clear ya aou gou fou figdisp isav gstimb
ikstop=13;
ikstop=0;
 vgconv=3e10/rr;
% 'qui', keyboard
...
%            set(hf1,'pos',[  265   427   359   504])
            set(hf1,'pos',[ 270   500   359   423])
     h1=figure;
%      set(h1,'pos',[  12   424   246   504])
      set(h1,'pos',[  18   500   246   418])
      hf=figure; 
%            set(hf,'pos',[ 600   425   583   504])      
            set(hf,'pos',[ 605   500   583   429])      
    end 
...
%      plot(lF,abs(autc),lF,abs(autc1),lF,abs(autcp),lF(fiz),abs(autc(fiz)),'*'), pausak
      ...
      plot(fov(isol,1:ki-1),alv(isol,1:ki-1),'.-',Fint(ki),alv2(fiz),'wo','linewidth',2),
      hold on, 
      if inew==1, plot(fpid,aexp,'w--','linewidth',2), end
      plot(Fint(ki),alv2,'x'), grid
      subplot(212)
       if isol>1
        semilogy(fovp,govp,'--'), hold on
       end
      semilogy(fov(isol,1:ki-1),gov(isol,1:ki-1)/vgconv,'.-',Fint(ki),gv2(fiz)/vgconv,'wo','linewidth',2), 
      hold on, 
      if inew==1, semilogy(fpid,gexp/vgconv,'w--','linewidth',2),       end
      semilogy(Fint(ki),gv2/vgconv,'x'), grid, 
      figure(hf)
...
clear gstim zstim
  ...
  if sa(1)>1
   for isa=1:sa(1)
    ...
    iaccet=0;
%    if length(fitum)==0 | length(fitup)==0
%     [du,imimi]=min(abs(aoud(isa,:)));
     afit=aoud(isa,1:2);
     if length(find(isnan(afit)==1))==0
      ffit=foud(isa,1:2);
      co=polyfit(ffit,afit,1);
      ro=roots(co);
      if abs(1-ffit(1)/ro)<.2 & ro<ffit(1)
       gfit=log10(goud(isa,1:2));
       co=polyfit(ffit,gfit,1);       
       gstim(1)=10^polyval(co,ro);
       zstim(1)=ro;
      else
       gstim(1)=1e20;
       zstim(1)=NaN;
     ...
     afit=aoud(isa,end-1:end);
     if length(find(isnan(afit)==1))==0
      ffit=foud(isa,end-1:end);
      co=polyfit(ffit,afit,1);
      ro=roots(co);
      if abs(1-ffit(end)/ro)<.2 & ro>ffit(end)
       gfit=log10(goud(isa,end-1:end));
       co=polyfit(ffit,gfit,1);       
       gstim(3)=10^polyval(co,ro);
       zstim(3)=ro;
      else
       gstim(3)=1e20;
       zstim(3)=NaN;
   ...
   if length(Fint)>3
     afit1=aoud(isa,1:end-1);
     afit2=aoud(isa,2:end);
     proa=afit1.*afit2;
     fiz=find(proa<0)-1;
     if ifp==-10
%      figure
%      plot(proa), hold on, plot(fiz,0,'wo'), pausak
     end
     ...
     for kz=fiz
      ...
      afit=aoud(isa,pu);
      ffit=foud(isa,pu);
      gfit=log10(goud(isa,pu));
      co=polyfit(ffit,afit,1);
      ro=roots(co);
      co=polyfit(ffit,gfit,1);
      gsi(icz)=10^polyval(co,ro);      
      zsi(icz)=ro;      
     end
      if exist('gsi')
       [gstim(2),imi]=min(gsi);
...
[gstimb(isa),imimi]=min(gstim);
if ifp==-10 & icontr==1
figure, subplot(211), plot(Fint,aoud(isa,:),zstim,0,'wo')
subplot(212), semilogy(Fint,goud(isa,:),zstim,gstim,'wo'), 
title([' iaccet = ', num2str(iaccet)])
pausak
end
if length(find(gstim~=1e20))>0
[du,imi]=min(gstim);
if imi~=2
 iaccet=1
end 
...
%     if length(fitup)*length(fitum)>0 
     if length(fitup)*length(fitum)>0  | iaccet==1
      iaoud=iaoud+1
      aou(iaoud,:)=aoud(isa,:);
      gou(iaoud,:)=goud(isa,:);
      fou(iaoud,:)=foud(isa,:);
...
al_shiv=zeros(sa(1),1);
del_Freav=zeros(sa(1),1);
GRA=1;
for kg=fibor
 mi=mimiv(kg);
 xxd=fou(kg,:);
 figu=find(isnan(xxd)==0);
 if mi==1
  figu=figu(1:GRA+1);
  fial=1;
 else
   figu=figu(end-GRA:end);
   fial=GRA+1;
 end
...
% keyboard
 ...
 pd=polyder(cf);
 ay=polyval(pd,ro);
 del_Freav(kg)=al_shiv(kg)/ay;

 [du,imi]=min(abs(yy));
 rov=roots(cf);
 [du,iro]=min(abs(rov-xx(imi)));
 ro=rov(iro);
 fzstim(kg)=ro;
 
 [dumi,fizd]=sort(abs(xx-ro));
 distim(kg)=dumi(1)/xx(fizd(1));
 
 fiz=fizd(1:GRA+1);
 cg=polyfit(xx(fiz),zz(fiz),GRA);

 gastim(kg)=2*polyval(cg,ro);
 
 if ifp==-10
  subplot(211)
  plot(xx,yy,xx,polyval(cf,xx),ro,0,'wo'), 
  subplot(212)
  plot(xx,zz,xx,polyval(cg,xx),ro,gastim(kg)/2,'wo'), pausak 
 end
end
...
if exist('gastim')==1
 ...
 [du,imis]=min(gastim); 
 if ifp==-10  
  figure, plot(fou',aou',fzstim,0,'o'), pausak  
  fi=find(gastim<1e19);
  figure, semilogy(fou',gou',fzstim(fi),gastim(fi),'o') , pausak
 end
...
fize=find(isav<0);
fizep=find(isav>0);
[du,imiz]=min(gstimb(fize));
fiaccet=sort([fizep fize(imiz)]);
end
   for isa=1:fibor
      iaoud=isa;
      aou(iaoud,:)=aou(isa,:)+al_shiv(isa);
      fou(iaoud,:)=fou(isa,:)+del_Freav(isa);
   end
...
% nmascel=5
 ...
 fcov=1000*lambda;
 if ifp==-10
   figure, subplot(211), plot(fcov*foud',aoud','.-'), grid
   subplot(212), semilogy(fcov*foud',2*goud'/vgconv,'.-'), grid
  title(' Curves to be seached old ')
