Par=Ps.Par;
%if Ps.ipeak==1
  clear Kospd Kosmd Koszpd Koszmd KospdAny KosmdAny Kosp Kosm  Koszp Koszm 
%end  
...
%   ' forma ', pausak
  ...
  %shad
  %pausak
   if ifii(ndis)<=0 & igainshape==0 & condi==1 & shad~=8
%   if sha==1.5
%    shamem=sha;
%    sha=1;
%   end 
...
%     'wvp ver Bes', keyboard
      ...
      if iany==3 & sha==4
       F1=.5*besr(ik2,:,inu-1).*besr(ik1,:,imu-1).*RdR.*Bmm(:,jmu,jnu)';
       Fi1y(jmu,jnu,ik1,ik2)=sum(F1);
       F2=.5*besr(ik2,:,inu+1).*besr(ik1,:,imu+1).*RdR.*Bpp(:,jmu,jnu)';
       Fi2y(jmu,jnu,ik1,ik2)=sum(F2);
%       F3=.5*besr(ik2,:,inu-1).*besr(ik1,:,imu+1).*RdR.*Amp(:,jmu,jnu)';
       F3=.5*besr(ik2,:,inu-1).*besr(ik1,:,imu+1).*RdR.*Apm(:,jmu,jnu)';       
       Fi3y(jmu,jnu,ik1,ik2)=sum(F3);
%       F4=.5*besr(ik2,:,inu+1).*besr(ik1,:,imu-1).*RdR.*Apm(:,jmu,jnu)';
       F4=.5*besr(ik2,:,inu+1).*besr(ik1,:,imu-1).*RdR.*Amp(:,jmu,jnu)';
       Fi4y(jmu,jnu,ik1,ik2)=sum(F4);      
%      'controllo iany 3', pausak
      end 
%      'controllo', pausak
      if iztm==1
  ...
  for jmu=pimu
   ...
   for jnu=pimu
	...
	% modifica!!!!!!
       ...
       if iany==3 & sha==4
	      S=Fi1y(imu,inu,:,:);
	      KmmA(pmu,pnu)=reshape(S,npk,npk)/fnor;
	      S=Fi2y(imu,inu,:,:);
	      KppA(pmu,pnu)=reshape(S,npk,npk)/fnor;
	      S=Fi3y(imu,inu,:,:);
	      KmpA(pmu,pnu)=reshape(S,npk,npk)/fnor;
	      S=Fi4y(imu,inu,:,:);
...
%       SEG
       kad=SEG*(P*(KmmA+smu*snu*KppA)+ (smu*KmpA + snu*KpmA) );
%       pausak
       KDUc=[KDUc kad];
      end
      KDU=[KDU; KDUc];
      ...
      KtotD(:,:,ipv)=KDU;     
        ...
        KAmduD=.5*dito*KtotD(:,:,1)*diae1;
        KApduD=.5*dito*KtotD(:,:,2)*diae1;        
        KAmduAny=KAmduD;
        KApduAny=KApduD;
%    'qui or\,', keyboard

%'qui Sit fine', keyboard
    end
...
if iany==3
 KospdAny (ndis,:,:)=KApduAny*alons^2;
 KosmdAny (ndis,:,:)=KAmduAny*alons^2;
end

%' igainshape vett ', keyboard
  if igainshape==0 & ayi~=0
...
%' fione raggoi', keyboard
   ...
   if length(siz)==3
    ...
    if iany==3
     for iir=[1:lxi]
      Kosp(:,:,iir+lxi) = KospdAny(iir,:,:);
      Kosm(:,:,iir+lxi) = KosmdAny(iir,:,:);
      Koszp(:,:,iir+lxi) = KospdAny(iir,:,:)*0;
      Koszm(:,:,iir+lxi) = KosmdAny(iir,:,:)*0;      
     end
    end
