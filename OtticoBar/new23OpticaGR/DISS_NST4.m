...
%     [du,imimi]=min(abs(aoud(isa,:)));
   ...
   if length(Fint)>3
     afit1=aoud(isa,2:end-2);
     afit2=aoud(isa,3:end-1);
     proa=afit1.*afit2;
     fiz=find(proa<0);
     icz=0;
     clear gsi
...
if exist('gastim')==1
 ...
 if ifp==-10  
  ...
  figure, plot(fou',gou',fzstim,gastim,'o') , pausak
 end
...
if isav(imis)<0
    ...
    subplot(212), semilogy(fou',gou')  
    
     ' soluzione fuori intervallo' , keyboard
    end

    diss_lasv
    if abs(1-Gsov(imis)/(gastim(imis)))<.2
      Gsov=gastim(imis);
      Fsov=fzstim(imis);
      del_Frea=-1;
    end
     ' soluzione fuori intervallo: controllo validita' , keyboard
    return  
