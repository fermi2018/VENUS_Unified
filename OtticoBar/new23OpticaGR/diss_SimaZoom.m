...
for kf=1:size(fou,1)
 ...
 LFit=length(If0)-1;
 cog=polyfit(Fint(If0),log10(gai(If0)),LFit);
...
%   nmascel=1:nma;
 ...
 if ifp==-10
  ...
  hold on
  semilogy(1000*lambda*Fintmod,2*gaim/vg,'sr')
  a=axis;
  a(3)=2*min(min(gou'/vgconv));
  a(4)=2*max(max(gou'/vgconv));
  axis(a)
...
if length(imodi)>=length(nmascel)
 Nsov=imodi(nmascel);
else
 Nsov=imodi;
end
