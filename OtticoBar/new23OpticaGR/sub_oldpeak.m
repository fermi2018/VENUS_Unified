...
%   end
   ...
   if isfield(Ps,'ifpstop')==1
    ...
    if length(kkP)>0
       if Ps.ifpstop==1
        plot(KK(fiFI),Ape(fiFI),'ro')  
       end 
     ApeM=Ape(fiFI);
     [du,iso]=sort(1./ApeM);
     if isfield(Ps,'NumPeak')
      NumPeak=Ps.NumPeak;
     else
      NumPeak=5;
     end
     if length(iso)>NumPeak
      kkP=kkP(iso(1:NumPeak));
      ApeM=ApeM(iso(1:NumPeak));
         if Ps.ifpstop==1
           plot(kkP,ApeM,'gp')  
         end
      end   
...
%c   subplot(211), plot(lco*fou(:,puf),aou(:,puf),lco*fso,zeromo,'wo'), grid;
   ...
   if dista>10
    if idyn==1
     subplot(212), semilogy(lco*fou(puf,:)',2*gou(puf,:)'/NQW_ef,lco*Fsov,Gsov/vgconvPS/NQW_ef,'wo');
     Mg=(Gsov/vgconvPS/NQW_ef);
    else 
     subplot(212), semilogy(lco*fou(puf,:)',2*gou(puf,:)'/vgconv,lco*Fsov,Gsov/vgconvPS/NQW_ef,'wo');
     Mg=(Gsov/vgconvPS/vgconv);     
    end 
   else
    subplot(212), plot(lco*fou(puf,:)',gou(puf,:)',lco*Fsov,NQW_ef*Gsosa/vg,'wo');
   end
      a=axis;
      a([3 4])=[min(Mg)*.8 max(Mg)*1.5];
   axis(a)
   grid
pausak
