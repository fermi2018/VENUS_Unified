function [ga,la,zet,Ez,nz,Gaqw,NQW_ef,az,uL,E_Temp,z_Temp]=fiezCMMn(lam0,L_i,n_i1,iat,icav,fiQW,ifp,dlam,NPlam,rr,kt,ibast,par_grat,a_i,LaPrec,TolMod);
...
n_i=n_i1(:,1);
ifpc=-4;  %NON controllo soluzione
...
%	'Lma',keyboard
 if isnan(TolMod)==0
  [du,ilap]=min(abs(LV-LaPrec));
  Gconf=GV(ilap)/(1+TolMod);
  [G0,imi]=min(GV);
  if G0>Gconf
   G0=GV(ilap);
   LAM=LV(ilap);
...
figure, plot(zet,Ez.*conj(Hz)*1.5,'r',zet,nz), pausak
...
zte=linspace(Lsu(1),Lsu(end-2),201);
...
coe=polyfit(zet(fiM),log10(E2(fiM)),10);
...
global idyn
if ifp==-10 & idyn==1
figure, semilogy(zet,E2,zte,Em,'r',zet(fiM),E2(fiM),'r.'), pausak
...
if iplotCa==1
zqw1=sum(L_i(1:fiQW(1)))/1000;
zqw2=sum(L_i(1:fiQW(end)))/1000;
%[du,imi]=min(abs(zet-zqw));
fitu=find(zet>zqw1 & zet<zqw2);

I0=abs(Ez).^2;
Ime=2*mean(I0(fitu));
I=log10(I0/Ime)+max(real(nz(:,1)));
I(1,fize)=NaN;
h=figure, 
vg=uL(2)/uL(1);
