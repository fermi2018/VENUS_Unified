...
%GMA=1e6;
%GMA=1e12;
clear ya aou gou fou figdisp isav gstimb fstimb gstimc fstimc
ikstop=13;
ikstop=0;
 vgconv=3e10/rr;
% 'qui', keyboard
...
%     [du,imimi]=min(abs(aoud(isa,:)));
     ...
     if length(find(isnan(afit)==1))==0
      ...
      ro=roots(co);
      if abs(ffit(1)-ro)/dF<preInt & ro<ffit(1)
     ...
     if length(find(isnan(afit)==1))==0
      ...
      co=polyfit(ffit,afit,1);
      ro=roots(co);
      if abs(ffit(end)-ro)/dF<preInt & ro>ffit(end)
...
[gstimb(isa),imimi]=min(gstim([1 3]));
fsd=zstim([1 3]);
fstimb(isa)=fsd(imimi);
istimb(isa)=imimi;
gstimc(isa)=gstim(2);
fstimc(isa)=zstim(2);
if ifp==-10 & icontr==1
...
fi0=find(gstimc<3e14 & gstimc<=gstimb/1.2);
fitot=fi0;

fib=find(gstimb<gstimc*1.2 & gstimb<3e14 & gstimb>1e13);
ifce=zeros(size(fi0));
GStim=gstimc(fi0);
FStim=fstimc(fi0);
if fib>0
 ifbo=istimb(fib);
 iftot=([ifce ifbo]);
 [fitot,isof]=sort([fi0 fib]);
 iftot=iftot(isof);
 GStimd=[GStim gstimb(fib)];
 FStimd=[FStim fstimb(fib)];
 GStim=GStimd(isof);
 FStim=FStimd(isof);
else 
 iftot=ifce;
...
goudis=goud(fi0,:);
 ...
 if ifp==-10
  ...
  title(' Curves  all ')
  figure, subplot(211), plot(fcov*foudis',aoudis','.-'), grid
  subplot(212), semilogy(fcov*foudis',2*goudis'/vgconv,'.-'), grid
...
%  ifp=-4
%' capeoi questa prima dei campi', keyboard   
...
% ' prima qui nmaze', keyboard
%iso=iso(1:nmasce);
%if min(gso)>min(gstimb)
% [gad,fim]=min(gstimb);
% gso=[gso gad];
% fso=[fso fstimb(fim)];
% [du,iso]=sort(gso); 
