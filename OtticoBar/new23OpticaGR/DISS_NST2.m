...
inor=2;   % =1 guarda la norma degli autovettori, =0 valori, =2 derivata
if ~exist('ipost')
 ipost=0;
end 
   isav_Az=0;
...
%      'Amix', pausak
   ...
   %   autc(ifii)=log10(abs(filtA(Amix,so)))'*log10(abs(filtA(Amr,so)));
      ...
      autca(ifii)=abs((filtA(Amix,so))'*(filtA(Amr,so))); 
...
%      autdiff=abs(diff(log10(abs(Amix(fP))))'*diff(log10(abs(Amr(fP))))); 
%      autdiff=abs(diff(abs(Amix(fP)))'*diff(abs(Amr(fP)))); 
%      autdiff=abs(diff(real(Amix(fP)))'*diff(real(Amr(fP)))); 
      autc(ifii)=1./autdiff;
