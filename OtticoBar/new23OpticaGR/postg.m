...
%c   subplot(211), plot(lco*fou(:,puf),aou(:,puf),lco*fso,zeromo,'wo'), grid;
   ...
   Gso=Gsosa/vgconvPS;
    ...
    subplot(212), plot(lco*fou(puf,:)',gou(puf,:)'/vg1,lco*Fsov,Gso,'wo');
   end
...
% icasa=1;
fig=find(gsov>0);
Gcm=2*gsov(fig)/vgconv;
figure, plot(M2v(fig),Gcm,'ro'), pausak
dlam=lambda./(1+fsov(fig));
dlam=lambda*fsov*1000;

%figure, plot(M2v(fig),dlam,'ro'), pausak
figure, semilogy(dlam(fig),Gcm,'go'), grid, pausak
figure, plot(dlam(fig),M2v(fig),'co'), title(' M2 '), grid, pausak
...
Del2=dlam(fig);
Gcm2=Gcm;
figure, semilogy(Del1,Gcm1,'ro',Del2,Gcm2,'gs'),
