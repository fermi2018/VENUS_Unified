...
%      z0c=input(' distanza ff in cm = ');
      ...
      teRd=asin(KKt*rr);
      Rd=z0./cos(teRd);
      Fd=-1./(Rd.*tan(teRd)).*exp(-j*k0*Rd+j*pi/4);
      lK=length(KK);
      lKv=diff(ldap);
      ldap1=ldap;
      ldap1=ldap;
...
%       pumi=(1:lK)+imi1*lK;
       ...
       AnMz=[zeros(nk1max-length(AnEd),1); AnMd];
...
%       cos2=exp(i*(mm-1/2)*pi/2);
       ...
       AnMz=[zeros(nk1max-length(AnMd),1); AnMd];
