...
vel(N,T,ro_ini,zeta,xroI,fimaxi,lfi_inp,par_in,r_pil,nvar,...
...
% ' qui dentro', keyboard
 ...
 zdis0=zdis;
% zdis=[zm([1 end]) zdis(fi)]; 
[du,imi]=min(abs(zdis-zm(1)));
zdis(imi)=zm(1);
[du,imi]=min(abs(zdis-zm(end)));
zdis(imi)=zm(end);
 %ndz=length(zdis)-2;
