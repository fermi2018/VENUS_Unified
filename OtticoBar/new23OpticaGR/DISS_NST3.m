  ...
  if sa(1)>1
   for isa=1:sa(1)
    ...
    if length(fitum)==0 | length(fitup)==0
     ...
     if imimi==1 | imimi==length(Fint)
      [du,imig]=min(abs(goud(:,imimi)));
       if imig==isa 
        iaccet=1;
       end 
     end
...
for kg=1:length(isav)
 ...
 fz(kg)=ro;
 ga(kg)=polyval(cg,ro);
% plot(xx,yy,xx,polyval(cf,xx)), pausak 
end


if ifp==-10  
figure, plot(fou',aou',fz,0,'o'), pausak  
figure, plot(fou',gou',fz,ga,'o') , pausak
end
[du,imis]=min(ga);
if isav(imis)<0
  mial=min(abs(aou(imis,:)));
    al_shi=-mean(mial*1.2);
    Fi1=Fint(1:size(alv,2));
    coau=polyfit(Fi1,aou(imis,:),1);
