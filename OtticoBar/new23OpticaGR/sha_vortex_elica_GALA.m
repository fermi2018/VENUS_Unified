...
if SPP_GA==1
 nScalini=Par.Nscalini-1;
 nScalini0=Par.Nscalini;
 if imag(nScalini0)~=0
  if ndise==2
   nScalini=real(nScalini0)-1;
  else
   nScalini=imag(nScalini0)-1;
  end
   nS2=nScalini+2;   
 end
 
 PhaSteps=linspace(0,2*pi,Ngiri*(nS2));
  if isfield(Par,'sfaLuna')==1 & ndise==1
