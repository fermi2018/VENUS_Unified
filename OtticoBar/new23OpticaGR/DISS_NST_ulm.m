...
%      autc(ifii)=abs((filtA(Amix,so)))'*abs((filtA(Amr,so)));
      autc(ifii)=log10(abs(filtA(Amix,so)))'*log10(abs(filtA(Amr,so)));
      autca(ifii)=(filtA(Amix,so))'*(filtA(Amr,so));
...
%     if du<.9 & icontrc==1
      ...
      subplot(211),
      plot(fov(isol,1:ki-1),alv(isol,1:ki-1),'.-',Fint(ki),alv2(fiz),'wo','linewidth',2), hold on, plot(Fint(ki),alv2(iasd),'x'), grid
      subplot(212)
      semilogy(fov(isol,1:ki-1),gov(isol,1:ki-1),'.-',Fint(ki),gv2(fiz),'wo','linewidth',2), hold on, plot(Fint(ki),gv2(iasd),'x'), grid, pausak
      hf=figure; 
            set(hf,'pos',[ 600   425   583   504])

      semilogy(xla,abs(Adis),xla,abs(Amr),'w.',xla,abs(Amix0),'wo'), 
      a=axis;
