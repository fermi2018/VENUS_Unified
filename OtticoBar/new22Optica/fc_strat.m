function [S11,S12,S21,S22]=fc_strat(beta2,Nlayer,nindex_in,nindex,nindex_out,d,lambda0,pol)
 ...
 % Computation of S11 of the cell
   if imag(nindex_out)<=0
     kx_out=f_psqrt(k00^2*nindex_out^2-beta2);
   else % lo strato _out e' attivo
     kx_out=asqrt(k00^2*nindex_out^2-beta2);
   end
   if imag(nindex_in)<=0
     kx_in=f_psqrt(k00^2*nindex_in^2-beta2);
   else % lo strato _out e' attivo
     kx_in=asqrt(k00^2*nindex_in^2-beta2);
   end  
  if strcmp(pol,'TE') 
   rFre=(kx_in-kx_out)./(kx_in+kx_out);
  elseif strcmp(pol,'TM')
    rFre=(kx_out/nindex_out^2-kx_in/nindex_in.^2)/(kx_out/nindex_out^2+kx_in/nindex_in^2);
  end
   S11=rFre;
   argS11=angle(S11);
   S22=-rFre;
   ...
   if strcmp(pol,'TE') % impedance factor
    impfact=2*sqrt(kx_out*kx_in)/(kx_out+kx_in);
   elseif strcmp(pol,'TM')
    impfact=2*nindex_in*nindex_out*sqrt(kx_out*kx_in)/(kx_out*nindex_in^2+kx_in*nindex_out^2);
   end   
   S12=impfact;
   argS12=angle(S12); 
   S21=S12;
...
% La function calcola la matrice S di una statificazione dielettrica generica con mezzi terminali qualsiasi. 
% I materiali possono avere perdite o guadagno.
...
if imag(nindex_out)<=0
 ...
 if imag(nindex(Nlayer))<=0
    kx(Nlayer,:)=f_psqrt(k00^2*nindex(Nlayer)^2-beta2);
...
for ind=Nlayer:-1:2
    ...
    if imag(nindex(ind-1))<=0
        kx(ind-1,:)=f_psqrt(k00^2*nindex(ind-1)^2-beta2);
    else % lo strato Nlayer e' attivo
...
for ind=Nlayer:-1:2
%for ind=1:Nlayer:-1:2
    Gap(ind,:)=Gam(ind+1,:).*exp(-j*2*kxrev(ind,:)*drev(ind));
    Gam(ind,:)=(rFrerev(ind,:)+Gap(ind,:))./(1+rFrerev(ind,:).*Gap(ind,:));
    argS11=argS11+angle(Gam(ind,:))-angle(Gap(ind,:))-2*real(kxrev(ind,:))*drev(ind);
end % for ind=Nlayer:2
Gap(1,:)=Gam(2,:).*exp(-j*2*kxrev(1,:)*drev(1));

Gam(1,:)=(rFrerev(1,:)+Gap(1,:))./(1+rFrerev(1,:).*Gap(1,:));
argS11=argS11+angle(Gam(1,:))-angle(Gap(1,:))-2*real(kxrev(1,:))*drev(1);
