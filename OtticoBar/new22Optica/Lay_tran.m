...
%      'ICI TJ lay_tran lithog', keyboard

if i1d==0
%      'ICI TJ lay_tran', keyboard
end
     else
       lay_mes(2)=count-1;
       paTJ.puTJ0=lay_mes(1):lay_mes(2);
     end


%      'ICI ultimo TJ lay_tran', keyboard
...
%**************************************
     ...
     case 'grating'
      ...
      [~,~,metamaterial]=fipar(remainder,ictP,count,ipar,sha,'metamaterial=',ictr);
	  if metamaterial==1
	   sha=-4;
       shav(count-1,ictr)=sha;	   
	  end
%'cpomt', keyboard
...
%'nAlGa', keyboard
        n(count,ictr)=nGaAsP(lambdaNot,AlCont)-i*Absorp*100/(2*kNoll);
%         n(count,ictr)
%        'n', pausak
        xm(count,ictr)=AlCont;
        xw(countw,ictr)=AlCont;
...
%      'materi', keyboard        
	...
	%         ipar(count,3,ictP)=ictr-1;
        ...
        Absorp=str2num(secondWord);
        n(count,ictr)=abs(ni)-i*Absorp*100/(2*kNoll);
        xm(count,ictr)=-10;
        xw(countw,ictr)=-10;
%'laytran'
%Absorp
...
for k=1:length(DDdu)
 ...
 for ki=1:l
  ...
  elseif strcmp(char(dch(ki)),'Ref_1.6')
   dchn{ki}='AlOx';    
%   'quiii', keyboard
  elseif strcmp(char(dch(ki)),'Ref_-1.6')
   dchn{ki}='AlGaAsSide';    
%   'quiii', keyboard
  end
 end 
