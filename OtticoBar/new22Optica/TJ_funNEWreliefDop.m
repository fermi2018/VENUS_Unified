function [dv_out,nv_out,av_out,iauto_out, shavet_out, fmlstot_out, anyf_out, xm_out, Dop_out, radii_out, ifield_out]=...
TJ_fun(ifp,PaTJ,L_i,n_in,aitot_in,iauto, shavet_in, fmlstot, anyf, xm_in, Dop, radii, ifield)
...
Le=[L_i(1); sum(L_i(puTJ)); L_i(pui)];

nex=[n_i(1,:); n_lateral*ones(size(n_i(1,:))); n_i(pui,:)];
Ltop=flipud(Le);
Les=cumsum(Ltop);
ntopE=flipud(nex);
...
Dop_out=[Dop(1,:); 0*ones(Lf,size(loc,2)); Dop(puB,:)];
'DOP', keyboard
...
%  'kt', keyboard
  ...
  n_extI=interp1(Lint_int,ne_int,Dfinal);
  
  nFIT=flipud(n_extI);
  nRest=n_i(puB,:);
  nv_add=[n_i(1); nFIT(1:end-1,:); nRest];
  
  sh_extI=round(interp1(Lint_int,sh_int,Dfinal));
  
  nFIT=flipud(sh_extI);
  nRest=shavet(puB,:);
  sh_add=[shavet(1); nFIT(1:end-1,:); nRest];  
%  'shavet', keyboard
  rag_I=interp1(Lint_int,ae_int,Dfinal);  
  nFIT=flipud(rag_I);
...
[fr,fc]=find(nv_out==1.001);
if length(fr)>0
nv_out(fr(2),fc-1)=nv_out(fr(2),fc-2);
nv_out(fr(end)+1,fc-2)=nv_out(fr(end)+1,1);
end
