function [Ez,Hz,nz,zet,Gaqw,NQW_ef,az,uL,velm]=Flam_field(L_in,n_i1,iat,fiQW,la0,rr,kt,ibast,par_grat,g0,Nx_in,a_i);
...
s11=-iT*Tref(2,1);
s12=iT;
s21=Tref(1,1)-Tref(1,2)*iT*Tref(2,1);
s22=Tref(1,2)*iT;
...
GaU=(rr-n1)/(rr+n1);
%
GaAtTop=s22+s12*s21*GaU/(1-s11*s22);
...
EznM=nz(:,1).'.*abs(Ez.*uLongT).^2.*ze;
EzfM=abs(Ez).^2.*ze;


n_med=real(sum(EznM)/sum(EzfM));
Lef=sum(EzfM)/max(Ezf.*uLong)*1e-6;
...
clight=3e8;
vgconvPS=clight/rr*100;
CP=clight/n_med/(2*Lef*uL(1));

h=6.626e-34;
hbar=h/(2.*pi);
om0=2*pi*1e6*clight./la;
fPdif=1./(Teff.*hbar.*om0); %-- tibaldi
...
velm.Lm=g0*vgconvPS*uL(1);
velm.vlambda=la*1000;
velm.Gamma_zTOT=uL(2);
velm.Gamma_z=uL(3:end);
velm.NQW=uL(2)/uL(1);
velm.nindexQW=real(na);
velm.nindexRef=rr;
velm.fPES=fPES;
velm.fPdif=fPdif;
