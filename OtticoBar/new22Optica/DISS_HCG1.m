...
%            set(hf1,'pos',[  265   427   359   504])
            set(hf1,'pos',[ 270   277   359   423])
     h1=figure;
%      set(h1,'pos',[  12   424   246   504])
      set(h1,'pos',[  18   274   246   418])
      hf=figure; 
%            set(hf,'pos',[ 600   425   583   504])      
            set(hf,'pos',[ 605   273   583   429])      
    end 
...
%      'cont fpo', keyboard
       ...
       Ae=aexp(end);
       Ge=gexp(end);
       Tagg=1./[(abs(alv2/Ae))+ abs(log10(gv2/Ge))]';
       Tagg=0;
       Tagg=1./[(abs(alv2-Ae))]';
%       Tagg=1./[(abs(alv2-Ae))+ abs(log10(gv2/Ge))]';
...
%      Adis(:,icok)=Adum/sqrt(abs(Adum'*Adum));
     ...
     [du,fiz]=max(abs(autu))
%     aimag=abs(imag(autc1));
%     [du,fiz]=max(1./aimag+autc);
%     pausak
%     [du,fiz]=max(abs(autu));
...
%      close(h1:hf)
       ...
       if ks1==2
        Am(:,1,isol)=Amr*median(abs(Amr));
       end 
        Am(:,ks1,isol)=Amix0*median(abs(Amix0));
...
nma0=s(1);
 ...
 if ifp==-10
  ...
  subplot(212), semilogy(fcov*fou',gou'/vgconv,'.-'), grid
...
% gg0=polyval(cog,ze);
 ...
 tt0=polyval(cot,ze);
  if icontr>=1
   ...
   semilogy(xg*falam,yg,ze*falam,gg0,'wo'), grid, pausak
%   close(he), clear he
  end
...
%   semilogy(Adispla),   
   expla=' Coeff to be interpolated,  Red dots: closest point ';
   title(expla)
...
%'qui nmaze', keyboard
gso=gso(iso);
if length(gso)>nmascea
 gso(nmascea+1:end)=-gso(nmascea+1:end);
end 
