function [Tg1,Tg2,Tsav,Tg3,Tg4]=Teq1_VORTEX(KK,lambda,thicki,par_grat,rr,mbv,ifp,segem,icrit,isca,Tlm_in)
...
tetav=tev/pi*180;
phiv=linspace(0,IntPhi*180,IntPhi*NDIS_fi)';
...
phir=IntPhi*phif*pi/180;
dfi=diff(phir(1:2))/(pi);
dfi1=dfi*faPhi*ones(size(phir));
if ifit==1
...
% fattori angolari traformata cilindrica progressimo-regressivo
...
%        ' cont qui', keyboard
 ...
 if IntPhi==1 
  T3s{ite}=T3d(puord,puord);   
  T4s{ite}=T4d(puord,puord);   
 end 
...
fi=find(abs(Ttotca)<1e-12);
Ttotca(fi)=0;

fi=find(abs(Ttotsa)<1e-12);
Ttotsa(fi)=0;
