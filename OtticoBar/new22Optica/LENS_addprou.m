...
function [dilu,ailu,nilu,fstu]=lens_add(th_in,h,r,N,nl,ifp,Npair,Rflat,St_soglia)
%' entro add', keyboard
%ifp=-10
 if h==0
  Ni=abs(Npair);
...
st=.5;
st=.25;
if exist('St_soglia') & length(St_soglia)>0
 st=St_soglia;
end 
%st=.01;
%'st', keyboard
radig=sort([r:-st:st]);
raend=radig(end)+1e-10;
radig(end)=raend;
xsa=xu;
