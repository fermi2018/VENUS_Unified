...
vg=faGam;
%vg=3e10/rr;
fi_x=input(' punto x? (return tutti)  ')
if length(fi_x)==0
 fi_x=1:size(demm,1);
end
fi_y=input(' punto y? (return tutti)  ')
if length(fi_y)==0
 fi_y=1:size(demm,2);
end
...
%figure(f1)
%subplot(211), plot(Dpp(kc),gainv,'o'), ylabel('Thres. Gain')
%subplot(212), plot(Dpp(kc),dlav,'o'), ylabel('Detuning')
%xlabel(YL)
%punt=1:length(Dpp);
...
figure(h)
subplot(211), hold on, plot(zev(kpl)*fafr,0,'rx') ,
subplot(212), hold on, semilogy(lai(kpl),gai(kpl),'rx')


figure(f2)
set(f2,'pos', [800   PY   675   462])
subplot(211),
 plot(abs(Aov{kpl}),['.-r']), hold on,
 plot(abs(Aqv{kpl}),['-g'])
 title([' Gain= ',num2str(gainv(kpl)),'     Dlam=  ',num2str(laver(kpl)),'     M2=  ',num2str(M2(kpl))])
 xlabel(' k index')
...
subplot(412), semilogy(Dx,Gv,'.-'), title(' Gain')
wl=1000*lambda./(1+Dv);
