% cercare st= a circa linea 770 

function [dilu,ailu,nilu,fstu]=lens_tip(th_in,h,r,N,nl,ifp,Npair,Rflat,ST_soglia)
%ST_soglia=.1;
...
% N=ceil(per0/stdu);
 ...
 N1=ceil(N*thmi(1)/sum(thmi));
 N2=ceil(N*thmi(2)/sum(thmi));
...
% dux=sqrt((R+yi).^2-(R+y(k)).^2);
 dux=sqrt((R+yi-h).^2-(R-h+y(k)).^2);
 rdux=real(dux);
 idux=imag(dux);
 fi00=find(rdux<=rmax & idux<=1e-4 & rdux>=0);
 ifi0=0;
 if length(fi00)==0
  ifi0=1;
%  fi00d=find(yi-y(k)>0);
...
%' prima aggiusto', keyboard
dxu=diff(xu); 
 firid=find(abs(dxu)>st);
 maxr=max(xu(firid));
 radig0=radig;
 if length(maxr)>0
  fira=find(radig0>maxr);
  radig=radig0(fira);
 end
...
clear ailu nilu dilu

for k=1:length(ailuss);
 if ailuss(k,1)~=ap
  %k
  %pausak
  ica=ica+1;  
  ap=ailuss(k,1);
  ailu(ica,1)=ailuss(k,1);
  fid=find(ailuss(:,1)==ap);
  disu=sum(diluss(fid,1));
  dilu(ica,1)=disu;
  nilu(ica,:)=niluss(k,:);
