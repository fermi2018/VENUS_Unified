...
%   pes00=[dkt; dkt(end)];
  ...
  for ik=1:length(Nkv)
    ...
    kiniz=kfin;
    dkt=diff(k(1:2));
    pes00=[pes00; dkt*ones(npuntik,1)];
  end  
  
  if isfield(Ps,'IKmodo')
   IKmodo=Ps.IKmodo;
   if IKmodo==-1
     p01=linspace(pes00(1),pes00(end),length(pes00))';
     pes00=p01;
     KK=cumsum(p01);
   end
  end
