...
%vn=linspace(n2,n1,21);
%for indn=1:length(vn)
%  FunctionalTE(indn)=f_WeightFunctionEffectiveMedium(vn(indn),RTE0,Geometry.n_in,Geometry.n_out,Geometry.th,Parameters.vLam,Parameters.theta*pi/180,'TE');
%  [S11temp] = f_ComputeSlab(vn(indn),Geometry.n_in,Geometry.n_out,Geometry.th,Parameters.vLam,Parameters.theta*pi/180,'TE');
%  S11test(indn)=S11temp;
%  FunctionalTM(indn)=f_WeightFunctionEffectiveMedium(vn(indn),RTE0,Geometry.n_in,Geometry.n_out,Geometry.th,Parameters.vLam,Parameters.theta*pi/180,'TM');
%end
%[dum,GuessTE]=min(FunctionalTE);
%[dum,GuessTM]=min(FunctionalTM);

%OPTIONS=optimset('Display','iter','TolFun',1e-9,'TolX',1e-5);
OPTIONS=optimset('Display','off','TolFun',1e-9,'TolX',1e-5);
...
% n2TE=fminsearch(@(X)f_WeightFunctionEffectiveMedium(X,RTE0,Geometry.n_in,Geometry.n_out,Geometry.th,Parameters.vLam,Parameters.theta*pi/180,'TE'),vn(GuessTE),OPTIONS);
% n2TM=fminsearch(@(X)f_WeightFunctionEffectiveMedium(X,RTM0,Geometry.n_in,Geometry.n_out,Geometry.th,Parameters.vLam,Parameters.theta*pi/180,'TM'),vn(GuessTM),OPTIONS);
n2TE=fminsearch(@(X)f_WeightFunctionEffectiveMedium(X,RTE0,Geometry.n_in,Geometry.n_out,Geometry.th,Parameters.vLam,Parameters.theta*pi/180,'TE'),nTEBornWolf(1),OPTIONS);
n2TM=fminsearch(@(X)f_WeightFunctionEffectiveMedium(X,RTM0,Geometry.n_in,Geometry.n_out,Geometry.th,Parameters.vLam,Parameters.theta*pi/180,'TM'),nTMBornWolf(1),OPTIONS);
%[S11TE] = f_ComputeSlab(n2TE,Geometry.n_in,Geometry.n_out,Geometry.th,Parameters.vLam,Parameters.theta*pi/180,'TE');
%[S11TM] = f_ComputeSlab(n2TM,Geometry.n_in,Geometry.n_out,Geometry.th,Parameters.vLam,Parameters.theta*pi/180,'TM');

%disp('siamo dentro la f_EffectiveIndex :-)')
%keyboard
