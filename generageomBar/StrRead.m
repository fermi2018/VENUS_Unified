function [StrDD,StrTT,ParOpt]=StrRead(ParVet,str,iplot,Str1D)

filename=[str,'.str'];

%'Lay_dd ', keyboard
[ParDD,ParOpt,ParMore]=Lay_dd(filename);
...
% save falso22 % useful for fast debug using fastUnif1.m
if flgStop==1
    'salvato falso 22', keyboard
end
...
nref=ParOpt.n(:,2);
%fiox=find(real(nref>1) & abs(imag(nref))<1e-3 & real(nref<2))
% fiox=find(real(nref==1.6));
fiox=find(real(nref==1.1) | real(nref==1.6));
...
if isempty(pu_cav)
    ...
    StrDD.fiPassiv=length(Prag)-find(imag(Prag(:,1))>0);
    StrDD.fiPassiv=length(Prag)-fiox(end);
    StrDD.fiPassiv=length(Prag)-pu_cav(end)+5;
    StrDD.fiPassiv=fiox(end);
    StrDD.fiPassiv=pu_cav(end);
    StrDD.fiPassiv=pu_cav(1);
