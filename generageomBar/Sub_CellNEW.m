%' Entro Cell NEW', keyboard
indLong=1;
xini=0;

for indTrans=NCin:NCend;%size(material,2)
...
for indLong=2:size(material,1)-1 % loop on longitudinal (y) direction
    ...
    for indTrans=NCin:NCend;%size(material,2) % loop on transversal (x) direction
        mat=material{indLong,indTrans};
