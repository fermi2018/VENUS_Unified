...
%            repi=abs(rep(k,:)); % vector containing the #reps and #layers involved
            ...
            if rep(k,2)>0   % Positive: NO eq
                ...
                if isop==1  % enters here if an actual mireq.p exists
                    Pmat{kink,1}{1}=ParMore.mireq.p.Mateq;
                    kink=kink+1;
                    isop=0;
                elseif isop==-1
                    Pmat{kink,1}{1}=ParMore.mireq.n.Mateq;
                    kink=kink+1;
                    isop=0;
                end
            end
...
if length(ParMore.mireq.n2.x_eq)>0 && length(ParMore.mireq.n.x_eq)>0
    dux=[X_i(fineq,:); ParMore.mireq.n2.x_eq*[1 1]; ParMore.mireq.n.x_eq*[1 1]];
    dura=[Ra_i(fineq,:); zeros(1,size(Ra_i,2)); zeros(1,size(Ra_i,2))];
    duNA=[NA_i(fineq,:); 0*[1 1]; 0*[1 1]];
    duND=[ND_i(fineq,:); ParMore.mireq.n2.Dop_eq*[1 1]; ParMore.mireq.n.Dop_eq*[1 1]];
elseif length(ParMore.mireq.p.x_eq)>0 && length(ParMore.mireq.n.x_eq)>0
    dux=[X_i(fineq,:); ParMore.mireq.p.x_eq*[1 1]; ParMore.mireq.n.x_eq*[1 1]];
    dura=[Ra_i(fineq,:); zeros(1,size(Ra_i,2)); zeros(1,size(Ra_i,2))];
    duNA=[NA_i(fineq,:); ParMore.mireq.p.Dop_eq*[1 1]; 0*[1 1]];
    duND=[ND_i(fineq,:); 0*[1 1]; ParMore.mireq.n.Dop_eq*[1 1]];
elseif length(ParMore.mireq.n2.x_eq)>0
    dux=[X_i(fineq,:); ParMore.mireq.p.x_eq*[1 1]];
    dura=[Ra_i(fineq,:);  zeros(1,size(Ra_i,2))];
    duNA=[NA_i(fineq,:); ParMore.mireq.p.Dop_eq*[1 1]];
    duND=[ND_i(fineq,:); ParMore.mireq.n.Dop_eq*[1 1]];
elseif length(ParMore.mireq.n.x_eq)>0
    dux=[X_i(fineq,:); ParMore.mireq.n.x_eq*[1 1]];
    dura=[Ra_i(fineq,:); zeros(1,size(Ra_i,2))];
    duNA=[NA_i(fineq,:); ParMore.mireq.p.Dop_eq*[1 1]];
    duND=[ND_i(fineq,:); ParMore.mireq.n.Dop_eq*[1 1]];
else
