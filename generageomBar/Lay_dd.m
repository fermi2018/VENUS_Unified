            ...
            while isequal(typemat,'END_data')~=1
                ...
                switch typemat
                    ...
                    case 'grating'
                        ...
                        [ictP,ipar,Delta]=fipar(remainder,ictP,count,ipar,sha,'Delta=',ictr);
...
%                            keyboard
                        ...
                        for pcounces=counces;
                            ...
                            radii.arrayd{12}=Delta;
                        end

						   radii.a(count-1,1)=Rx;
%                           'ICI grat lay_new',   keyboard
            ...
            if isfield(radii,'arrayd')==1
                for pcounces=counces;
                    ...
                    radii.a(pcounces,ictrar)=radii.arrayd{7}(1);
                    radii.b(pcounces,ictrar)=radii.arrayd{8}(1);
                    radii.c(pcounces,ictrar)=radii.arrayd{12}(1);
                end
            end
            %  disp(' fine array')
            %  pausak
                ...
                case 'Reference'
                    ...
                    switch type
                        ...
                        case 'AlGaAs'
                            ...
                            AlCont=str2num(secondWord); % COMMENT ON AlCont!!!
                            if AlCont==0
                                AlCont=1e-8;
                            end
                        ...
                        case 'GaAsP'
                            ...
                            nref=real(nGaAsP(lambdaNot,AlCont));    
                    ...
                    while isempty(type)==0
                        ...
                        switch type
                            ...
                            case 'GaAsP'
                                ...
                                n(count,ictr)=real(nGaAsP(lambdaNot,AlCont))-i*Absorp*100/(2*kNoll);
                                xm(count,ictr)=AlCont;
                                xw(countw,ictr)=AlCont;
                            case 'Ref'
                                materi{ictr}=type;
...
rag=radii.a;
purag=Pun_cont(puDD); ParDD.xm=xm(purag,2:end);
DDrag=rag(purag,:);
ParDD.rag=DDrag;
...
for k=1:length(DDdu)
    ...
    for ki=1:l
        if strcmp(char(dch(ki)),'Ref_1')
            dchn{ki}='vacuum';
        elseif strcmp(char(dch(ki)),'Ref_1.6') | strcmp(char(dch(ki)),'Ref_1.1')
            dchn{ki}='AlOx';
        ...
        elseif strcmp(char(dch(ki)),'InGaAs')
            %   dchn{ki}='AlOx';
        elseif strcmp(char(dch(ki)),'GaAsP')
            %   dchn{ki}='AlOx';
        else
            dchn{ki}='vacuum';
            %   'quiii', keyboard
...
IPar(1:length(IPard(iDD:end,:)),1:size(IPard,2))=IPard(iDD:end,:);
ParDD.IPar=IPar;
if ~exist('ParMore')
    ParMore.r_passiv=[];
    ParMore.mesh_passiv=[];
