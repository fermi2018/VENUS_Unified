function [f,fp,fs,Nfun,MI,MIs,MId]=SEM_Nver(Lv,Nv,zi,kv)
...
for indPatch=1:length(PatchInfo)
    ...
    Mi=diag(2./(2.*vn+1))./PatchInfo(indPatch).dudx; % Gram matrix (Mass matrix)
    Ki=(Mi)*D2(1:PatchInfo(indPatch).Ntot,1:PatchInfo(indPatch).Ntot)*PatchInfo(indPatch).dudx^2; % Integrale Der2 * fun (Stiffness Matrix)
